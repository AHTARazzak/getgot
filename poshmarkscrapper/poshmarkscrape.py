...
def getthebrand(labels):
    for brand in labels:
        ...
        if not os.path.exists(directoryname+"catalogue.csv"):
            append_list_as_row(directoryname+"catalogue.csv",["Brand", "ItemCode","URL","Price","Size","Catagories","Colour","Time"])
            ...
            for item in itemurls:
                ...
                if not os.path.exists(itemcode):
                    ...
                    if len(size)>0:
                        ...
                        sizeis="N/A"
                    append_list_as_row(directoryname+"catalogue.csv",[brand, itemcode,item,thepriceis,sizeis,catagoriesis,colouris,datetime.datetime.now()])
